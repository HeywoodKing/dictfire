...
class DictFire:
    ...
    def parse(self, content):
        ...
        if code == 0:  # Success
            ...
            number_flag = 68
            print('\033[1;31m{} \033[0m'.format('#' * number_flag))
            print('\033[1;31m# \033[0m')
            print('\033[1;31m# \033[0m {0}'.format(t))
            if re.match('[ \u4e00 -\u9fa5]+', src) is None:
                # 不包含中文
                src_columns = number_flag
            else:
            ...
            if re.match('[ \u4e00 -\u9fa5]+', tgt):
                ...
                tgt_columns = number_flag

            print('\033[1;31m# \033[0m')
            lines = math.ceil(src_columns / number_flag)
            for line in range(lines):
                start_index = line * number_flag
                end_index = start_index + number_flag
                print('\033[1;31m# \033[0m {0}'.format(src[start_index: end_index]))

            # print('\033[1;31m# \033[0m')
            lines = math.ceil(tgt_columns / (number_flag - 2))
            for line in range(lines):
                start_index = line * (number_flag - 2)
                end_index = start_index + (number_flag - 2)
